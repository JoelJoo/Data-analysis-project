/*
première ligue sql analysis challenge
*/
...
FROM (
SELECT HomeTeam as Team, FTHG as TotalGoal FROM PL23
UNION ALL
SELECT AwayTeam as Team, FTAG as TotalGoal FROM PL23
) as CombinedGoal
GROUP BY Team
...
FROM (
SELECT HomeTeam AS Team, FTAG as TotalConceledGoal FROM PL23
UNION ALL
SELECT AwayTeam AS Team, FTHG as TotalConceledGoal FROM PL23
...
DROP TABLE IF exists PL23BestAttackAndDefense
CREATE TABLE PL23BestAttackAndDefense 
(TeamName varchar(250),
ScoredGoal int,
ConceledGoal int
)

INSERT INTO PL23BestAttackAndDefense (TeamName, ScoredGoal, ConceledGoal)
SELECT Team, SUM(ScoredGoal) as TotalScoredGoal, SUM(ConceledGoal) as TotalConceledGoal
FROM (
SELECT HomeTeam as Team, FTHG as ScoredGoal, FTAG as ConceledGoal FROM PL23
UNION ALL
SELECT AwayTeam as Team, FTAG as ScoredGoal, FTHG as ConceledGoal FROM PL23
) as GlobalTotalConceledGoal
GROUP BY Team;

SELECT * FROM PL23BestAttackAndDefense;